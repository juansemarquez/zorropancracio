...
function conectar() {
	...
	$con = mysql_connect($server, $user, $pass) or die("¡No se ha podido establecer la conexión con el servidor! " . mysql_error());
	//Elegimos la BD:
	$resp = mysql_select_db($db) or die("¡No se ha podido seleccionar la base de datos! " . mysql_error());
}
...
function validar_base($usr) {
	$sql="Select id_base from bases where id_base=\"$usr\"";
...
elseif($origen==23&&$destino==24) {$dist=593;}
if($dist==-1) {$dist=distancia($destino,$origen);}
return $dist;
} 
